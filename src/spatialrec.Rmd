...
MakeTableFromImages <- function(imgs) {
  ...
  print("Getting pixel values as table")
  dims <- dim(imgs[[names(imgs)[1]]])
  ans <- foreach(v = imgs, .combine = cbind) %do% as.vector(v)
  print(dim(ans))
  
  print("Doing rownames...")
  rn <- apply(expand.grid(1:dims[1], 1:dims[2], 1:dims[3]), 1, paste, collapse = "_")
...
## Averages images with similar genes
```{r}
AverageImages <- function(imgs, which.genes) {
  ans <- list()
  for(g in unique(which.genes)) {
    imgs.use <- names(imgs)[grepl(paste0("^",g), names(imgs))]
    for(name in imgs.use) {
      if(is.null(ans[[g]])) {
        ans[[g]] <- imgs[[name]]@.Data
      } else {
        ans[[g]] <- ans[[g]] + imgs[[name]]@.Data
      }
    }
    ans[[g]] <- ans[[g]]/length(imgs.use)
  }
...
```{r}
MakeVideoFromVector <- function(vec) {
  dims <- as.integer(strsplit(names(tail(vec,1)), "_")[[1]])
  array(vec, dims)
}
```

## Calculates the probability that each 
```{r}
CalcProbPixel <- function(tbl, cl) { 
  ans <- matrix(NA, nrow = nrow(tbl), ncol = length(cl))
  colnames(ans) <- names(cl)
  rownames(ans) <- rownames(tbl)
  for(j in 1:ncol(ans)) {
    print(paste0("j = ", j))
    ans[, j] <- apply(tbl, 1, dmvnorm, mean = cl[[j]]$muhat, sigma = cl[[j]]$sigmahat)
  }
  
  ans <- apply(ans, 1, function(x) {x / sum(x)})
