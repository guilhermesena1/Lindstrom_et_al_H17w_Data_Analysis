...
```{r}
cell.in.bin <- function(srt, levels) {
    ans <- matrix(NA, nrow = ncol(srt@data), ncol = nrow(levels))
    rownames(ans) <- colnames(srt@data)
    colnames(ans) <- paste0("bin_", rownames(levels))
    
    genes <- colnames(levels)
    genes <- genes[genes %in% rownames(srt@data)]
    for(i in rownames(levels)) {
        print(paste0("level ", i))
        ans[,paste0("bin_",i)] <- apply(srt@data[genes,] > 0, 2, 
            function(x){ 
                y <- levels[i,]
                tp <- sum(x &  y)
                tn <- sum(!x & !y)
