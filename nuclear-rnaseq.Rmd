...
diff.heatmap <- function(srt, mark, n = 20, p.cutoff = 1e-5) {
  ...
  DoHeatmap(srt, genes.use = topn$gene,  
            slim.col.label = TRUE, remove.key = TRUE, draw.line = T, cells.use = 1000)
}
```
...
```{r}
FigureBomb <- function(srt, diffexp, markers, out.dir) {
    # PCS
    ...
    # Features of top diff
     FeatureTopDiff(srt, diffexpr, paste0(out.dir, "/Feature_Plots/clusters"), n = 10)
...
```{r}
mcc.bin.scores <- function(df, levels) {
    ...
    for(i in rownames(levels)) {
        ...
        ans[,paste0("bin_",i)] <- apply(df[genes,] > 0, 2, 
            function(x){ 
               MCC(x, levels[i,])
            })
    }
...
BoxFeatureByCluster <- function(srt, which) {
  df <- data.frame(srt@data.info[, which])
  rownames(df) <- rownames(srt@data.info)
  colors <- gg_color_hue(length(unique(srt@ident)))
  ...
  ggplot(df, aes(cluster, feature)) + geom_boxplot(fill = colors)
}
```
...
impute.lasso <- function(srt, gene) {
    ...
    predict(fit, ta, s = fit$lambda.min)
}
```
...
ImputeMarkers <- function(srt, markers) {
    ...
    srt@imputed <- data.frame(sapply(as.character(markers), function(x) {impute.lasso(srt.14w.nephro, x)}))
